<form action="${initParam["urlpaypal"]}" method="POST" target="_top">
    <input type="hidden" name="business" value="fgonzalez@utdelacosta.edu.mx" />
    ...
    <c:forEach items="${sessionScope.cart}" var="pro">
        ...
        <input type="hidden" name="tax_cart" value="${total*.16}" />
        <input type="hidden" name="currency_code" value="${moneda}" />
        <input type="image" src="https://www.sandbox.com/es_ES/ES/i/btn/btn_buynow_LG.gif" border="0" name="submit" alt="PayPal, la forma rápida y segura de pagar en Internet." />
        <img alt="" border="0" src="https://www.sandbox.com/es_ES/ES/i/src/pixel.gif" width="1" height="1" />
</form>